...
def compute_phase_tuning(b_phase, b_timestamps, spike_times, spike_times_index, epochs):
    ...
    # Prepend zero to the spike_times_index
    spike_times_index_p = np.concatenate(([0], spike_times_index))
    num_units = len(spike_times_index_p) - 1

    # Loop through each unit
    for unit_num in range(num_units):
        # Extract spike times for the unit        
        unit_spike_times = spike_times[spike_times_index_p[unit_num]:spike_times_index_p[unit_num + 1]]
