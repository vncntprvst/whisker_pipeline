...
class Utils:
    ...
    # @staticmethod
    # def detect_movement_periods(trace, velocity_threshold=0.01, amplitude_threshold=None, duration=1, sample_rate=None, timestamps=None):
    #     """
    #     Detect periods of movement based on a threshold, and keep only movement periods
    #     that exceed the specified duration.

    #     Parameters:
    #     trace: np.ndarray
    #         The input behavior trace (1D array).
    #     velocity_threshold: float
    #         Threshold for detecting movement. No movement is below this threshold.
    #     duration: float
    ...
    #     Returns:
    #     np.ndarray
    #         A movement mask (boolean array) where True indicates movement periods
    #         that meet the duration criterion.
    #     """
    #     if timestamps is None and sample_rate is None:
    #         raise ValueError("You must provide either a sample_rate or timestamps.")

    #     # Detect movement periods based on the velocity threshold
    #     movement_mask = np.abs(np.diff(trace, prepend=trace[0])) > velocity_threshold

    #     # Calculate the minimum duration in samples or time
    #     if timestamps is not None:
    #         # Calculate duration in terms of timestamps
    #         time_diffs = np.diff(timestamps, prepend=timestamps[0])
    #         min_duration_samples = np.zeros_like(movement_mask, dtype=bool)

    #         # Identify movement periods and filter by duration
    #         filtered_movement_mask = np.zeros_like(movement_mask, dtype=bool)
    #         in_movement = False
    #         start_idx = 0
    #         accumulated_time = 0
    ...
    #                 # Start of a new movement period
    #                 start_idx = i
    #                 in_movement = True
    #                 accumulated_time = 0
    #             elif not movement_mask[i] and in_movement:
    #                 # End of a movement period
    #                 end_idx = i
    #                 if accumulated_time >= duration:
    #                     # Only keep the movement if it exceeds the duration threshold
    #                     filtered_movement_mask[start_idx:end_idx] = True
    #                 in_movement = False
    #             if in_movement:
    #                 accumulated_time += time_diffs[i]

    #         # Handle the case where the last movement period continues until the end
    #         if in_movement and accumulated_time >= duration:
    ...
    #         # Handle the case where the last movement period continues until the end
    #         if in_movement and (len(movement_mask) - start_idx) >= min_duration_samples:
    #             filtered_movement_mask[start_idx:] = True
                
    #     # Find periods of movement that meet the movement amplitude threshold
    #     if amplitude_threshold is None:
    #         amplitude_threshold = np.std(trace) #2 * 
    #     filtered_movement_mask = filtered_movement_mask | (np.abs(trace) > amplitude_threshold)

    #     return filtered_movement_mask
