function measurements = MeasurementsToH5(filename)
...
%  05/04/23 - VP - Adapted to export measurements to HDF5


[s,attr] = fileattrib(filename); % get abs path
if(s)
    % Load measurments
    measurements = Whisker.mexLoadMeasurements(attr.Name);

    % Save the measurements
    [fileDir,fileName]=fileparts(attr.Name);
    try
%         For Python: load with scipy.io.loadmat
        save(fullfile(fileDir, [fileName '.mat']), 'measurements')
    catch
        % For Python: load with h5py.File
%         save(fullfile(fileDir, [fileName '.h5']), 'measurements', '-v7.3')
        save(fullfile(fileDir, [fileName '.mat']), 'measurements', '-v7.3')
    end
else
    error(attr)
end
